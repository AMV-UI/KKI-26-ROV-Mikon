...
public:
  ...
  // get singleton
  static AP_Baro *get_singleton(void) { return _singleton; }

  // barometer types
  typedef enum { BARO_TYPE_AIR, BARO_TYPE_WATER } baro_type_t;
  ...
  float get_pressure(uint8_t instance) const {
    return sensors[instance].pressure;
  }
#if HAL_BARO_WIND_COMP_ENABLED
  // dynamic pressure in Pascal. Divide by 100 for millibars or hectopascals
  const Vector3f &get_dynamic_pressure(uint8_t instance) const {
    return sensors[instance].dynamic_pressure;
  }
#endif
#if (HAL_BARO_WIND_COMP_ENABLED || AP_BARO_THST_COMP_ENABLED)
  float get_corrected_pressure(uint8_t instance) const {
    return sensors[instance].corrected_pressure;
  }
#endif
  ...
  float get_temperature(uint8_t instance) const {
    return sensors[instance].temperature;
  }

  // get pressure correction in Pascal. Divide by 100 for millibars or
  // hectopascals
  float get_pressure_correction(void) const {
    return get_pressure_correction(_primary);
  }
  float get_pressure_correction(uint8_t instance) const {
    return sensors[instance].p_correction;
  }
  ...
  float get_altitude(uint8_t instance) const {
    return sensors[instance].altitude;
  }

  // get altitude above mean sea level
  float get_altitude_AMSL(uint8_t instance) const {
    return get_altitude(instance) + _field_elevation_active;
  }
  ...
  static void get_pressure_temperature_for_alt_amsl(float alt_amsl,
                                                    float &pressure,
                                                    float &temperature_K);
#endif

  // lookup expected temperature in degrees C for a given altitude. Used for
  // SITL backend
  ...
  float get_ground_pressure(void) const {
    return get_ground_pressure(_primary);
  }
  float get_ground_pressure(uint8_t i) const {
    return sensors[i].ground_pressure.get();
  }
  ...
  float get_external_temperature(void) const {
    return get_external_temperature(_primary);
  };
  ...
  static void SimpleUnderWaterAtmosphere(float alt, float &rho, float &delta,
                                         float &theta);
...
private:
  ...
  public:
    ...
    AP_Float xp;    // ratio of static pressure rise to dynamic pressure when
                    // flying forwards
    AP_Float xn;    // ratio of static pressure rise to dynamic pressure when
                    // flying backwards
    AP_Float yp;    // ratio of static pressure rise to dynamic pressure when
                    // flying to the right
    AP_Float yn;    // ratio of static pressure rise to dynamic pressure when
                    // flying to the left
    AP_Float
        zp; // ratio of static pressure rise to dynamic pressure when flying up
    AP_Float zn; // ratio of static pressure rise to dynamic pressure when
                 // flying down
  };
#endif
  ...
  struct {
    ...
    uint32_t last_change_ms; // last update time in ms that included a change in
                             // reading from previous readings
  ...
  AP_Float _specific_gravity; // the specific gravity of fluid for an ROV 1.00
                              // for freshwater, 1.024 for salt water
  AP_Float _user_ground_temperature; // user override of the ground temperature
                                     // used for EAS2TAS
  float _guessed_ground_temperature; // currently ground temperature estimate
                                     // using our best available source
  ...
  void probe_i2c_dev(AP_Baro_Backend *(*probefn)(AP_Baro &, AP_HAL::Device &),
                     uint8_t bus, uint8_t addr);
  void probe_spi_dev(AP_Baro_Backend *(*probefn)(AP_Baro &, AP_HAL::Device &),
                     const char *name);
  void probe_dev(AP_Baro_Backend *(*probefn)(AP_Baro &, AP_HAL::Device &),
                 AP_HAL::Device *dev);
#if AP_BARO_ICM20789_ENABLED
  void probe_icm20789(uint8_t bus, uint8_t addr, const char *mpu_name);
  void probe_icm20789(uint8_t bus, uint8_t addr, uint8_t mpu_bus,
                      uint8_t mpu_addr);
  ...
  void probe_lps2xh_via_Invensense_IMU(uint8_t bus, uint8_t addr,
                                       uint8_t mpu_addr);
  ...
  */
  ...
  void
  update_thrust_filter(void); // update filtered throttle once per update cycle
  LowPassFilterFloat _thrust_filter; // low-pass filter for thrust compensation
  uint32_t _thrust_filter_last_update_us; // last update time for filter dt
                                          // calculation
