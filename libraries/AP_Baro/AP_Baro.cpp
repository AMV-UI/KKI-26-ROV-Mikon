...
#define AP_FIELD_ELEVATION_ENABLED                                             \
  !defined(HAL_BUILD_AP_PERIPH) && !APM_BUILD_TYPE(APM_BUILD_ArduSub)
#endif
...
// ground temperature and pressure
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("1_GND_PRESS", 2, AP_Baro, sensors[0].ground_pressure, 0,
                       AP_PARAM_FLAG_INTERNAL_USE_ONLY),

    // @Param: _GND_TEMP
    // @DisplayName: ground temperature
    // @Description: User provided ambient ground temperature in degrees
    // Celsius. This is used to improve the calculation of the altitude the
    // vehicle is at. This parameter is not persistent and will be reset to 0
    // every time the vehicle is rebooted. A value of 0 means use the internal
    ...
    // This is used to allow for automatic adjustment of the base barometric
    // altitude by a ground station equipped with a barometer. The value is
    // added to the barometric altitude read by the aircraft. It is
    // automatically reset to 0 when the barometer is calibrated on each reboot
    // or when a preflight calibration is performed.
    // @Units: m
    // @Increment: 0.1
    // @User: Advanced
    ...
    // @Description: This selects which barometer will be the primary if
    // multiple barometers are found
    ...
    // @Description: This selects the bus number for looking for an I2C
    // barometer. When set to -1 it will probe all external i2c buses based on
    // the BARO_PROBE_EXT parameter.
    // @Values: -1:Disabled,0:Bus0,1:Bus1,6:Bus6
    // @User: Advanced
    AP_GROUPINFO("_EXT_BUS", 7, AP_Baro, _ext_bus,
                 HAL_BARO_EXTERNAL_BUS_DEFAULT),

    // @Param{Sub}: _SPEC_GRAV
    // @DisplayName: Specific Gravity (For water depth measurement)
    // @Description: This sets the specific gravity of the fluid when flying an
    // underwater ROV.
    // @Values: 1.0:Freshwater,1.024:Saltwater
    // @Range: 0.98 1.05
    AP_GROUPINFO_FRAME("_SPEC_GRAV", 8, AP_Baro, _specific_gravity, 1.0,
                       AP_PARAM_FRAME_SUB),
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("2_GND_PRESS", 9, AP_Baro, sensors[1].ground_pressure, 0,
                       AP_PARAM_FLAG_INTERNAL_USE_ONLY),
...
// Slot 10 used to be TEMP2
    ...
    // @Volatile: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("3_GND_PRESS", 11, AP_Baro, sensors[2].ground_pressure,
                       0, AP_PARAM_FLAG_INTERNAL_USE_ONLY),
...
// Slot 12 used to be TEMP3
    ...
    // @Description: This sets the range around the average value that new
    // samples must be within to be accepted. This can help reduce the impact of
    // noise on sensors that are on long I2C cables. The value is a percentage
    // from the average value. A value of zero disables this filter.
    // @Units: %
    // @Range: 0 100
    // @Increment: 1
    AP_GROUPINFO("_FLTR_RNG", 13, AP_Baro, _filter_range,
                 HAL_BARO_FILTER_DEFAULT),

#if AP_BARO_PROBE_EXT_PARAMETER_ENABLED
    // @Param: _PROBE_EXT
    // @DisplayName: External barometers to probe
    // @Description: This sets which types of external i2c barometer to look
    // for. It is a bitmask of barometer types. The I2C buses to probe is based
    // on BARO_EXT_BUS. If BARO_EXT_BUS is -1 then it will probe all external
    // buses, otherwise it will probe just the bus number given in BARO_EXT_BUS.
    // @Bitmask:
    // 0:BMP085,1:BMP280,2:MS5611,3:MS5607,4:MS5637,5:FBM320,6:DPS280,7:LPS25H,8:Keller,9:MS5837,10:BMP388,11:SPL06,12:MSP,13:BMP581,14:AUAV
    // @User: Advanced
    AP_GROUPINFO("_PROBE_EXT", 14, AP_Baro, _baro_probe_ext,
                 HAL_BARO_PROBE_EXT_DEFAULT),
#endif

    // @Param: 1_DEVID
    // @DisplayName: Baro ID
    // @Description: Barometer sensor ID, taking into account its type, bus and
    // instance
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("1_DEVID", 15, AP_Baro, sensors[0].bus_id, 0,
                       AP_PARAM_FLAG_INTERNAL_USE_ONLY),

#if BARO_MAX_INSTANCES > 1
    // @Param: 2_DEVID
    // @DisplayName: Baro ID2
    // @Description: Barometer2 sensor ID, taking into account its type, bus and
    // instance
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("2_DEVID", 16, AP_Baro, sensors[1].bus_id, 0,
                       AP_PARAM_FLAG_INTERNAL_USE_ONLY),
#endif
    ...
    // @Description: Barometer3 sensor ID, taking into account its type, bus and
    // instance
    // @ReadOnly: True
    // @User: Advanced
    AP_GROUPINFO_FLAGS("3_DEVID", 17, AP_Baro, sensors[2].bus_id, 0,
                       AP_PARAM_FLAG_INTERNAL_USE_ONLY),
#endif
    ...
    // @Description: User provided field elevation in meters. This is used to
    // improve the calculation of the altitude the vehicle is at. This parameter
    // is not persistent and will be reset to 0 every time the vehicle is
    // rebooted. Changes to this parameter will only be used when disarmed. A
    // value of 0 means the EKF origin height is used for takeoff height above
    // sea level.
    // @Units: m
    // @Increment: 0.1
    // @Volatile: True
    ...
    // @Description: This is the maximum acceptable altitude discrepancy between
    // GPS altitude and barometric presssure altitude calculated against a
    // standard atmosphere for arming checks to pass. If you are getting an
    // arming error due to this parameter then you may have a faulty or
    // substituted barometer. A common issue is vendors replacing a MS5611 in a
    // "Pixhawk" with a MS5607. If you have that issue then please see
    // BARO_OPTIONS parameter to force the MS5611 to be treated as a MS5607.
    ...
    // @Description: Thrust scaling in Pascals. This value scaled by the
    // normalized thrust is subtracted from the barometer pressure. This is used
    // to adjust linearly based on the thrust output for local pressure
    // difference induced by the props.
    // @Range: -300 300
    // @User: Advanced
    ...
    // @Description: Low-pass filter cutoff frequency for thrust compensation.
    // This smooths the throttle input to prevent rapid baro altitude changes
    // during throttle transients. Set to 0 to disable filtering.
    // @Range: 0 10
    // @Units: Hz
    // @User: Advanced
...
void AP_Baro::calibrate(bool save) {
  ...
  if (hal.util->was_watchdog_reset()) {
    GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                  "Baro: skipping calibration after WDG reset");
    return;
  }
  ...
  for (uint8_t i = 0; i < _num_sensors; i++) {
    if (sensors[i].calibrated) {
      GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Barometer %u calibration complete",
                    i + 1);
      num_calibrated++;
    }
  }
...
void AP_Baro::update_calibration() {
  ...
  for (uint8_t i = 0; i < _num_sensors; i++) {
    if (healthy(i)) {
      float corrected_pressure = get_sealevel_pressure(
          get_pressure(i) + sensors[i].p_correction, _field_elevation_active);
...
float AP_Baro::get_external_temperature(const uint8_t instance) const {
  ...
  if (_last_external_temperature_ms != 0 &&
      AP_HAL::millis() - _last_external_temperature_ms < 10000) {
  ...
  // The reason for not just using the baro temperature is it tends to read
  // high, often 30 degrees above the actual temperature. That means the EAS2TAS
  // tends to be off by quite a large margin, as well as the calculation of
  // altitude difference between two pressures reporting a high temperature will
  // cause the aircraft to estimate itself as flying higher then it actually is.
...
/*
  wrapper around hal.i2c_mgr->get_device() that prevents duplicate devices being
  opened
 */
bool AP_Baro::_i2c_sensor_is_registered(uint8_t bus, uint8_t address) const {
  for (int i = 0; i < _num_sensors; ++i) {
    if (AP_HAL::Device::make_bus_id(AP_HAL::Device::BUS_TYPE_I2C, bus, address,
                                    0) ==
...
#define GET_I2C_DEVICE_PTR(bus, address)                                       \
  _i2c_sensor_is_registered(bus, address)                                      \
      ? nullptr                                                                \
      : hal.i2c_mgr->get_device_ptr(bus, address)

// probe for an I2C barometer.
void AP_Baro::probe_i2c_dev(AP_Baro_Backend *(*probefn)(AP_Baro &,
                                                        AP_HAL::Device &),
...
// probe for an I2C barometer
void AP_Baro::probe_spi_dev(AP_Baro_Backend *(*probefn)(AP_Baro &,
                                                        AP_HAL::Device &),
                            const char *name) {
...
void AP_Baro::probe_dev(AP_Baro_Backend *(*probefn)(AP_Baro &,
                                                    AP_HAL::Device &),
                        AP_HAL::Device *dev) {
  if (dev == nullptr) {
    return;
...
void AP_Baro::probe_lps2xh_via_Invensense_IMU(uint8_t bus, uint8_t addr,
                                              uint8_t mpu_addr) {
  auto *i2c_dev = GET_I2C_DEVICE_PTR(bus, addr);
  AP_Baro_Backend *backend =
      AP_Baro_LPS2XH::probe_InvensenseIMU(*this, *i2c_dev, mpu_addr);
  if (!_add_backend(backend)) {
    delete i2c_dev;
  }
}
...
void AP_Baro::probe_icm20789(uint8_t bus, uint8_t addr, uint8_t mpu_bus,
                             uint8_t mpu_addr) {
  auto *i2c_dev = GET_I2C_DEVICE_PTR(bus, addr);
...
void AP_Baro::_probe_icm20789(AP_HAL::I2CDevice *i2c_dev,
                              AP_HAL::Device *mpu_dev) {
  if (i2c_dev == nullptr) {
    return;
  }
  if (mpu_dev == nullptr) {
...
// #if AP_BARO_ANALOG_ENABLED
//   _add_backend(AP_Baro_Analog::probe(*this, 14));
//   RETURN_IF_NO_SPACE;
// #endif
  ...
  const int8_t serial_port =
      AP::externalAHRS().get_port(AP_ExternalAHRS::AvailableSensor::BARO);
  if (serial_port >= 0) {
  ...
  case AP_BoardConfig::PX4_BOARD_PX4V1:
    ...
    probe_i2c_dev(AP_Baro_MS5611::probe, HAL_BARO_MS5611_I2C_BUS,
  ...
  case AP_BoardConfig::PX4_BOARD_AEROFC:
    ...
    probe_i2c_dev(AP_Baro_MS5607::probe, HAL_BARO_MS5607_I2C_BUS,
  ...
  if (_ext_bus >= 0) {
    ...
    probe_i2c_dev(AP_Baro_KellerLD::probe, _ext_bus,
                  HAL_BARO_KELLERLD_I2C_ADDR);
    RETURN_IF_NO_SPACE;
#endif
#else
#if AP_BARO_MS5611_ENABLED
...
void AP_Baro::_probe_i2c_barometers(void) {
  ...
  if (AP_BoardConfig::get_board_type() == AP_BoardConfig::PX4_BOARD_PIXHAWK2) {
    // for the purpose of baro probing, treat CubeBlack internal i2c as
    // external. It has no internal i2c baros, so this is safe
...
void AP_Baro::update(void) {
  ...
  for (uint8_t i = 0; i < _num_sensors; i++) {
    if (sensors[i].healthy) {
      ...
      if (!is_positive(ground_pressure) || isnan(ground_pressure) ||
          isinf(ground_pressure)) {
      ...
      if (sensors[i].type == BARO_TYPE_AIR) {
...
#endif
        altitude = get_altitude_difference(sensors[i].ground_pressure,
                                           corrected_pressure);
        ...
        altitude = (sensors[i].ground_pressure - corrected_pressure) / 9800.0f /
                   _specific_gravity;
      }
...
#define MASK_LOG_ANY 0xFFFF
  ...
  if (sensors[_primary].healthy != old_primary_healthy) {
    if (AP::logger().should_log(MASK_LOG_ANY)) {
      const LogErrorCode code = sensors[_primary].healthy
...
bool AP_Baro::healthy(uint8_t instance) const {
  // If the requested instance was outside max instances it is not healthy (it
  // doesn't exist)
...
void AP_Baro::update_field_elevation(void) {
  ...
  if (now_ms - _field_elevation_last_ms >= 1000) {
    if (is_zero(_field_elevation_active) && is_zero(_field_elevation)) {
      ...
      if (!armed && AP::ahrs().get_origin(origin)) {
        ...
        if (is_zero(_field_elevation_active)) {
          _field_elevation_active = 0.001f; // prevent zero value so that we
                                            // don't keep trying to auto-set
        }
        new_field_elev = true;
      }
      ...
      if (!armed) {
        ...
        _field_elevation.notify();
        GCS_SEND_TEXT(MAV_SEVERITY_ALERT,
                      "Failed to Set Field Elevation: Armed");
      }
    }
  }
  if (new_field_elev && !armed) {
    ...
    update_calibration();
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Field Elevation Set: %.0fm",
                  _field_elevation_active);
  }
#endif
}
...
bool AP_Baro::arming_checks(size_t buflen, char *buffer) const {
  ...
  if (_alt_error_max > 0 && gps.status() >= AP_GPS_FixType::FIX_3D) {
    ...
    // note the addition of _field_elevation_active as this is subtracted in
    // get_altitude_difference()
    const float alt_pressure =
        get_altitude_difference(SSL_AIR_PRESSURE, get_pressure());
    const float error = fabsf(alt_amsl - alt_pressure);
    if (error > _alt_error_max) {
      hal.util->snprintf(buffer, buflen,
                         "GPS alt error %.0fm (see BARO_ALTERR_MAX)", error);
      return false;
    }
  }
#endif
...
AP_Baro &baro() { return *AP_Baro::get_singleton(); }
